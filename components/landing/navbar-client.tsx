    ...
    <header className="fixed top-0 left-0 right-0 z-50 flex justify-center pt-5">
      ...
      >
        ...
        <Link href="/" className="flex items-center gap-2 group mr-2">
          ...
          <span className="text-sm font-semibold tracking-tight text-foreground font-display">{siteName}</span>
        </Link>
                ...
                <div key={link.href} className="relative group">
                  ...
                  <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-[900px] bg-card border border-border rounded-md shadow-md opacity-0 pointer-events-none scale-[0.97] origin-top transition-all duration-200 group-hover:opacity-100 group-hover:pointer-events-auto group-hover:scale-100 z-50 flex overflow-hidden">
                    <div className="w-[260px] shrink-0 bg-muted/40 p-8 border-r border-border flex flex-col justify-start text-left">
                      <h4 className="font-display text-2xl font-semibold tracking-tight text-foreground leading-[1.25]">
                        Everything in one place.
                      </h4>
                      <p className="text-[12px] font-medium tracking-[0.015em] text-muted-foreground leading-relaxed mt-3">
                        One quiet assistant that remembers your work, sits in on your meetings, and acts when it matters.
                      </p>
                    </div>
                    <div className="flex-1 p-8 grid grid-cols-2 gap-x-8 gap-y-5 text-left bg-card">
                      {featureNavItems.map((item) => (
                        <Link
                          key={item.id}
                          href={item.href}
                          className="group/item flex flex-col gap-1 hover:bg-muted/50 p-2.5 -m-2.5 rounded-sm transition-all duration-200"
                        >
                          <span className="text-[13px] font-semibold tracking-[0.01em] text-foreground font-display group-hover/item:text-foreground">{item.title}</span>
                          <span className="text-[11.5px] font-medium tracking-[0.015em] text-muted-foreground leading-normal">{item.desc}</span>
                        </Link>
                      ))}
                    </div>
                  </div>
            ...
            if (link.href === "/pricing") {
              return (
                <div key={link.href} className="relative group">
                  ...
                  <div className="absolute top-full left-0 right-0 h-2 z-50" />
                  <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-[580px] bg-card border border-border rounded-md shadow-md opacity-0 pointer-events-none scale-[0.97] origin-top transition-all duration-200 group-hover:opacity-100 group-hover:pointer-events-auto group-hover:scale-100 z-50 flex overflow-hidden">
                    <div className="w-[240px] shrink-0 bg-muted/40 p-8 border-r border-border flex flex-col justify-start text-left">
                      <h4 className="font-display text-2xl font-semibold tracking-tight text-foreground leading-[1.25]">
                      ...
                      {[
                        ...
                        >
                          <div className="flex items-baseline justify-between">
                            <span className="text-[13px] font-semibold tracking-[0.01em] text-foreground font-display group-hover/item:text-foreground">{item.title}</span>
                            <span className="text-[11.5px] font-semibold tracking-[0.015em] text-muted-foreground">{item.price}</span>
                          </div>
                          <span className="text-[11.5px] font-medium tracking-[0.015em] text-muted-foreground leading-normal">{item.desc}</span>
                        </Link>
                      ))}
                      <div className="border-t border-border pt-4 mt-1">
                        <Link href={link.href} className="text-[12px] font-semibold tracking-[0.01em] text-foreground font-display hover:text-muted-foreground flex items-center gap-1 group/btn w-fit">
                          Compare plans
            ...
            if (link.href === "/company") {
              return (
                <div key={link.href} className="relative group">
                  ...
                  <div className="absolute top-full left-0 right-0 h-2 z-50" />
                  <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-[520px] bg-card border border-border rounded-md shadow-md opacity-0 pointer-events-none scale-[0.97] origin-top transition-all duration-200 group-hover:opacity-100 group-hover:pointer-events-auto group-hover:scale-100 z-50 flex overflow-hidden">
                    <div className="w-[220px] shrink-0 bg-muted/40 p-8 border-r border-border flex flex-col justify-start text-left">
                      <h4 className="font-display text-2xl font-semibold tracking-tight text-foreground leading-[1.25]">
                        Built by creators.
                      </h4>
                      <p className="text-[12px] font-medium tracking-[0.015em] text-muted-foreground leading-relaxed mt-3">
      ...
      {/* Mobile drawer */}
      {isMobileMenuOpen && (
        <div className="absolute top-[calc(100%+4px)] left-4 right-4 border border-border bg-card/90 rounded-sm p-5 shadow-md flex flex-col gap-3 animate-in fade-in slide-in-from-top-2 duration-200 md:hidden max-h-[85vh] overflow-y-auto backdrop-blur-lg">
          {links.map((link) => {
            if (link.href === "/features") {
              return (
