...
const SlackIcon = () => (
  <svg viewBox="0 0 24 24" className="size-4 text-muted-foreground" fill="currentColor">
...
export function IntegrationCloud() {
  const integrations = [
    ...
    { icon: <Calendar className="size-4 text-muted-foreground" />,label: "Google Calendar", top: "68%", left: "20%", delay: "0.7s" },
    { icon: <FigmaIcon />,                                        label: "Figma",           top: "52%", left: "80%", delay: "2.1s" },
    { icon: <Mail className="size-4 text-muted-foreground" />,    label: "Gmail",           top: "72%", left: "70%", delay: "1.2s" },
    { icon: <FileText className="size-4 text-muted-foreground" />,label: "Notion",          top: "18%", left: "42%", delay: "2.8s" },
    { icon: <Mic className="size-4 text-muted-foreground" />,     label: "Sarvam AI",       top: "75%", left: "48%", delay: "0.3s" },
    { icon: <MessagesSquare className="size-4 text-muted-foreground" />, label: "Linear",   top: "45%", left: "8%",  delay: "1.9s" },
    { icon: <CheckSquare className="size-4 text-muted-foreground" />, label: "Jira",        top: "42%", left: "90%", delay: "3.2s" },
  ];

  return (
    <section className="relative w-full py-28 bg-muted/20 overflow-hidden border-y border-border">
      ...
      <div className="absolute inset-0 pointer-events-none hidden md:block">
        {integrations.map((item, idx) => (
          <div
            key={idx}
            className="absolute px-3 py-2 bg-card border border-border rounded-xl shadow-sm flex items-center gap-2 animate-[float_6s_ease-in-out_infinite]"
      ...
      <div className="max-w-9xl mx-auto px-6 relative z-10 text-center flex flex-col items-center gap-6">
        <div className="absolute -z-10 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 opacity-[0.06] dark:opacity-[0.02] pointer-events-none">
          <Image src="/keilhq.svg" alt="" width={200} height={200} className="dark:hidden" />
          <Image src="/keilhq-white.svg" alt="" width={200} height={200} className="hidden dark:block" />
        </div>

        <h2 className="text-3xl sm:text-4xl font-bold tracking-tight text-foreground max-w-xl leading-tight">
        ...
        <div className="flex md:hidden flex-wrap justify-center gap-3 mt-4">
          {integrations.slice(0, 6).map((item, idx) => (
            <div key={idx} className="flex items-center gap-2 px-3 py-2 bg-card border border-border rounded-full shadow-sm">
              {item.icon}
