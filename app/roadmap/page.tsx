            ...
            {columns.map((col) => (
              <div key={col.label} className={`rounded-2xl border p-6 flex flex-col gap-5 ${col.color}`}>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-semibold uppercase tracking-widest text-zinc-600">{col.label}</span>
                </div>
                <div className="flex flex-col gap-4">
                  {col.items.map((item) => (
                    <div key={item.title} className="bg-white rounded-xl border border-zinc-100 p-4 flex flex-col gap-1.5 shadow-sm">
